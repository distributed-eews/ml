...
def restart(input_data: json) -> str:
    ...
    # Insert station code
    station_list: str = redis_client.get(settings.REDIS_STATION_LIST_NAME)
    if station_list is None:
        redis_client.set(settings.REDIS_STATION_LIST_NAME, "")
        station_list = ""
    else:
        station_list = station_list.decode("UTF8")
    ...
    print("FROM REDIS: ")
    print(redis_client.get(f"{station_code}~data_p"))
    print(redis_client.get(f"{station_code}~data_s"))
